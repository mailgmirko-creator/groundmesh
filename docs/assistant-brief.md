...
- Balance-Engine boundary first: Before changing Balance Engine runtime, TSL loops, scoring/confidence fields, decision-support APIs, ledgers, dashboards, or action-selection behavior, read `docs/tsl/balance-engine-core-spec.md`; preserve human review, no person-worth scoring, no motive inference, no enemy labels, no autonomous allocation/publication/governance/punishment, no money/value optimization, and no remote authority without a command contract.
